...
int main(int argc, char **argv) {
  ...
  auto dec = xacc::getService<xacc::AcceleratorBufferDecorator>("error-correction-code");
  dec->setDecorated(buffer);
  std::cout << dec->size() << "\n";

std::cout << buffer->getChildren().size() << "\n";
  auto buffer2 = xacc::qalloc(2);
  dec->appendChild("ch", buffer);
  std::cout << buffer->getChildren("ch").size() << "\n";
  //buffer = dec;
...
CNOT 1 0
  ...
  xacc::OptFunction f(
      [&](const std::vector<double> &x, std::vector<double>& dx) {
        ...
        auto tmpBuffer = xacc::qalloc(dec->size());
        ...
        for (int i = 0; i < buffers.size(); i++) {
          ...
          dec->appendChild(fsToExec[i]->name(), buffers[i]);
        }
